...
# local_patient <- as.data.frame(dataset$patients[, c("subject_id", "anchor_year", "anchor_age")])
...
for (values in found_data_info) {
  ...
  if (1 == length(parsed$short)){
    ...
    cnpt <- load_concepts(parsed$short, src=dataset_name, aggregate=FALSE, interval=secs(1), id_type="patient")
    ...
    tbl = dataset[[parsed$table]][, c(parsed$sub_var, "subject_id", "valuenum")]
    gc()
    ...
    rm(cnpt_from_event, self_created_concept, itemid_mask)
    gc()
    ...
    # load ricu concept
    cnpt <- load_concepts(parsed$short[1], src=dataset_name, aggregate=FALSE, interval=secs(1), id_type="patient")
    cnpt_count = nrow(cnpt)
