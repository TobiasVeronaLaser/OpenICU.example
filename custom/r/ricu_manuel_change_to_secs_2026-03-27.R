if (!exists(".ricu_patch_applied", envir = .GlobalEnv)) {
  assign(".ricu_patch_applied", TRUE, envir = .GlobalEnv)
  
  ns <- asNamespace("ricu")
  
  old_load_ts_src_tbl <- get("load_ts.src_tbl", envir = ns)
  
  patched_load_ts_src_tbl <- function (x, rows, cols = colnames(x), id_var = id_vars(x), 
                                       index_var = ricu::index_var(x), interval = hours(1L), 
  ...
  environment(patched_load_ts_src_tbl) <- ns
  
  unlockBinding("load_ts.src_tbl", ns)
  assign("load_ts.src_tbl", patched_load_ts_src_tbl, envir = ns)
  lockBinding("load_ts.src_tbl", ns)
}